...
## first delineation method: dalponte 2016 -- seed and region growing algorithm
# Cite:
# Dalponte, M. and Coomes, D. A. (2016), 
# Tree-centric mapping of forest carbon density from airborne laser scanning and hyperspectral data.
# Methods Ecol Evol, 7: 1236-1245. doi:10.1111/2041-
#   210X.12575.
...
TH_tree <- c() # threshold below which a pixel cannot be a tree -- 5-15
TH_seed <- c() #growing threshold 1: A pixel is added to a region if its height is greater than the tree height multiplied by this value 0-1
TH_cr <- c() #growing threshold 2: a pixel is added to a region if its height is greater than the current mean height of the region multiplied by this value: 0-1
MAX_cr <- c() # max diameter of crown (in pixels) -- we don't want this to be a limiting factor: should this vary, or be held constant at a high value? Oaks can be 15m+. Default to 20 for now
...
for (i in 1:max_iteration){
  ...
  #--crown delineation parameters
   thTree <- signif(runif(1,5,15),4)
   thSeed <- signif(runif(1,0.01, 0.99),4)
   thCR <- signif(runif(1,0.01,0.99),4)
   maxCR <- signif(runif(1,2,25),4)
  #________________________________________________________________
   ##generate an auto delineation 
   TT <- tree_detection(las, lmf(ws = ttWS, hmin = ttHmin, shape = "circular"))
   crowns <- lastrees(las, dalponte2016(chm, TT, th_tree = 5, th_seed = thSeed,
                                        th_cr = thCR, max_cr = 20, ID = "treeID"))
...
dalponte_tune <- as.data.frame(cbind(iterate, TT_ws, TT_hmin,TH_seed,
                                     TH_cr,TH_tree,MAX_cr,Acc,nMC,nAC,
                                     P1,P2,P3,P4,P5,P6,P7,P8,P9,
                                     P10,P11,P12,P13,P14,P15))
                                    

write.csv(dalponte_tune, "dalponte_tune_300buf.csv")
